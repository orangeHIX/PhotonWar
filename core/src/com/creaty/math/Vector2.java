...
public class Vector2 extends com.badlogic.gdx.math.Vector2{
	 ...
	 * 
	 */
	private static final long serialVersionUID = -1500868039581976532L;

	public final static Vector2 X = new Vector2(1, 0);
	public final static Vector2 Y = new Vector2(0, 1);
	public final static Vector2 Zero = new Vector2(0, 0);
	
	public Vector2 rotate(float angle, Vector2 center) {
		float rad = angle * MathUtils.degreesToRadians;
		float cos = MathUtils.cos(rad);
		float sin = MathUtils.sin(rad);

		float newX = (this.x - center.x) * cos - (this.y - center.y) * sin
				+ center.x;
		float newY = (this.x - center.x) * sin + (this.y - center.y) * cos
				+ center.y;
